      ...
      <main className="container py-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blush/50 p-4 rounded-lg space-y-2">
              <p className="font-heading text-sm font-semibold">Perfect for:</p>
              <p className="text-sm font-body text-muted-foreground">
                {product.category === "haldi" && "Haldi ceremony, pre-wedding celebrations, yellow-themed events"}
                {product.category === "mehndi" && "Mehndi night, sangeet, festive celebrations"}
                {product.category === "bridal" && "Wedding day, reception, bridal photoshoots"}
                {product.category === "combo" && "Complete bridal look, wedding gift sets, trousseau"}
              </p>
            </div>
            ...
            <div className="border-t border-border pt-6 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center border border-border rounded-sm">
                  ...
                  <button onClick={() => setQty(qty + 1)} className="p-2 hover:bg-muted"><Plus size={16} /></button>
                </div>
              </div>
              <button
                onClick={() => { for (let i = 0; i < qty; i++) addToCart(product); }}
                className="w-full flex items-center justify-center gap-2 bg-gold text-primary-foreground py-3.5 rounded-sm font-body font-medium tracking-widest uppercase btn-glow hover:opacity-90 transition-all"
              >
            ...
            <p className="text-xs text-muted-foreground font-body text-center">
              🎨 Want customization? <a href="#" className="text-gold underline">Contact us on WhatsApp</a>
            </p>
          </div>
        </div>
